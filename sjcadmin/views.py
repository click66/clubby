...
def log_attendance(request):
    ...
    s = Student.objects.get(pk=request.POST.get('student_uuid'))
    a = Attendance.register_student(s, date=date.datetime.today().date())
    a.save()
    return redirect('student', student_uuid=s.uuid)
