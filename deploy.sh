...
scp -r "$BUILD_DIR/$FE_ARTIFACT_NAME" $SSH_USER@$SSH_HOST:$FE_ARTIFACT_PATH

echo "Starting release..."
ssh $SSH_USER@$SSH_HOST "bash -s" < ./release.sh $FE_ARTIFACT_PATH $BE_ARTIFACT_PATH $RELEASES_DIR $DEPLOY_DIR $SERVED_PATH
